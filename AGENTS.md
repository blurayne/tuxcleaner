...
**Directories are selectable and removed recursively in `analyze`.** Our fork extends the same relaxation to directories: a large directory under home can be selected and removed, recursively, through the normal explicit-selection and confirmation flow, instead of being refused outright as a directory. Authorized by the repository owner on 2026-08-18. Every existing protection still applies to a directory target (absolute, not `/` or home itself, under home, no `..`, denylist checked against every component, no symlink and no symlinked ancestor, at or above the size floor), and one new guard was added specifically because recursive removal is more destructive than a single file: a directory that is itself a git repository root (a `.git` entry lives directly inside it) is refused, so a user cannot destroy a project's `.git` by selecting the project's working directory as a whole. See `is_git_repository_root` in `src/executor.rs` and `PersonalFileRefusal::GitRepository` in `src/tui/view.rs`. Any branch carrying this change is fork-only and must never be sent upstream.

**Force gestures in `analyze` bypass the protected-location denylist, the git-repository-root guard, and the size floor for the owner's own files under home.** The analyze screen's Shift+D (force delete), Shift+Space, and `S` (force select) let the owner explicitly override `ANALYZE_MINIMUM_SIZE`, `is_git_repository_root`, and the `.ssh`/`.gnupg`/`.config`/`.git`/`go/pkg` denylist for a specific path, after an unchanged confirmation dialog (rendered distinctly, in red, naming exactly which guard was overridden). This directly relaxes the CLAUDE.md rule "Never delete `/`, the home directory, `.git`, `.config`, `.ssh`, or `.gnupg`" for the forced analyze path only; every other cleanup command (`clean`, `purge`, `uninstall`) and every non-analyze path are unaffected. Authorized by the repository owner on 2026-08-19. Force never overrides the hard checks that protect against deleting something outside the user's own files: the path being outside the home directory, the home directory itself, any `..` component, or the path being a symlink or having a symlinked ancestor -- these are enforced unconditionally, before and independently of the `force` flag, in `Executor::validate_personal_file_impl` (`src/executor.rs`) and `personal_file_selectability_impl` (`src/tui/view.rs`), so they are structurally impossible to bypass regardless of how forcing is invoked. `CleanupAction::RemovePersonalFile` carries the decision through execution and to history/JSON output as an additive `force: bool` field (`#[serde(default)]`, so older records without it still deserialize). Any branch carrying this change is fork-only and must never be sent upstream.
